...
// ─────────────────────────────────────────────────────────────────────────────
// Prompt content builders — shared across all tools
// ─────────────────────────────────────────────────────────────────────────────
...
function analyzeStoryPrompt(fw, _) {
  return `${header(fw)}
3. Load and execute the workflow at {project-root}/${fw}/workflows/analyze-story/
4. If a story file path was provided as an argument, use it directly; otherwise ask the user for the story file path
5. Follow ALL steps — output acceptance criteria, test scenarios, edge cases, and risk assessment`;
}

function analyzeProjectPrompt(fw, _) {
  return `${header(fw)}
3. Load and execute the workflow at {project-root}/${fw}/workflows/analyze-project/
4. Follow ALL steps — detect frontend framework, backend stack, database, auth flow, base URLs, API endpoints
5. Save results to {project-root}/${fw}/core/project.config.json
6. Display a summary of all detected project properties`;
}

function generateE2EPrompt(fw, _) {
  return `${header(fw)}
3. Load and execute the workflow at {project-root}/${fw}/workflows/generate-e2e/
4. If a story file path was provided as an argument, use it directly; otherwise ask the user for the story file path
5. Follow ALL steps — generate XLSX test cases + JavaScript Playwright POM tests → output to {output_folder}/{story-id}/`;
}
...
function generateReportPrompt(fw, _) {
  return `${header(fw)}
3. Load and execute the workflow at {project-root}/${fw}/workflows/generate-report/
...
4. Display all available skills in a formatted table: Skill Name | Description | Output | Phase
5. Group by phase order (Phase 0 → Phase 7, then extras)`;
}
